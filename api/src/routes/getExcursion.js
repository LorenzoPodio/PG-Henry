...
getExcursion.get("/", async (req, res, next) => {
  try {
    ...
    if (name) {
      ...
      excursionName.length
        ? res.status(200).send(excursionName)
      ...
      excursionLocation.length
        ? res.status(200).send(excursionLocation)
        : res.status(500).send("Excursion not found");
    } else if (date) {
      let day = date.charAt(0).toUpperCase() + date.slice(1);
      ...
      excursionDate.length
        ? res.status(200).send(excursionDate)
        : res.status(500).send("Excursion not found");
